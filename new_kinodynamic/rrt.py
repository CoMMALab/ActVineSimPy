...
class StatesStruct:
    ...
    def __init__(self, max_bodies, num_dynamic_objs):
        ...
        # Make all states active so every state is explored in RRT
        self._isactive = np.ones((self.init_size), dtype=bool)
    ...
    def extend_states(self):
        ...
        self._isactive = np.concatenate([self._isactive, np.ones(current_size, dtype=bool)], axis=0)
    ...
    def add_state(self, isactive, c_space, dstate, obj_positions, obj_dstate, 
                  bodies, time, bending_control, tip, parent_idx, num_children):
...
def sample_random_state(sst_params: SSTparams, batch_size, active_tips):
    '''
    Sample a random 2D state in the range defined by SSTparams.
    Returns shape (batch_size,) array, which hold the indices
    for the states that will be used for propagation
    '''

    x = np.random.uniform(sst_params.min_x, sst_params.max_x, batch_size)
    y = np.random.uniform(sst_params.min_y, sst_params.max_y, batch_size)
    theta = np.random.uniform(-np.pi, np.pi, batch_size)

    random_state = np.stack([x, y, theta], axis=1) # shaped (batch_size, 2)

    # Get the closest active state to the random state

    indices, dist2 = nearest_neighbor_all(sst_params, active_tips, random_state)
    nearest_idx = np.argmin(dist2, axis=1)
    nearest_tip_idx = indices[nearest_idx]

    return nearest_tip_idx


def cspace_to_tip(params: VineParams, batch_size, cspace: np.ndarray, n_bodies: np.ndarray, 
    ...
    FIXME: CSPACE.SHAPE[0] == 0, which is being passed as the batch size; why is this?
    """
    ...
    def get_last_body_length(cspace: np.ndarray, n_bodies: int):
        ...
        # # print("\nRUNNING LAST BODY LENGTH")

        # print(n_bodies, n_bodies.shape)

        # # print(num_bodies)

        # last = n_bodies - 1
        # prev_last = torch.clamp(last - 1, min=0)
        # last = torch.clamp(last, min=0)

        # # print(last.item(), prev_last)
        ...
        # dist = torch.sqrt((last_x - prev_x).pow(2) + (last_y - prev_y).pow(2))

        # return torch.where(n_bodies <= 1, torch.zeros_like(dist), dist)
    ...
    # assert n_bodies.shape == (batch_size, 1)
...
def rollout(sst_params, simparams, batch_size, 
    ...
    print(f"Steps to iter: {steps_to_iter}")
    print(f"num // denom = record_every: {sst_params.δs} // {(simparams.grow_rate * simparams.dt)} = {record_every}")
    ...
    obj_position_record = np.zeros((history_size, batch_size, simparams.obj_mass.size(0), 3), dtype=np.float32)
    obj_dstate_record = np.zeros((history_size, batch_size, simparams.obj_mass.size(0), 3), dtype=np.float32)
    ...
    if (last_index_filled <= 0): print(f"Last index filled is messed up by params")  
...
def rrt(sst_params: SSTparams, sim_params: VineParams, init_obj_pose,
        tree, iters=1000, callback=None):

    using_dynamic_obstacles = False if init_obj_pose.size == 0 else True
    ...
    if not tree:
        ...
        _ = tree.add_state(isactive=True,
                            c_space=cspace,
    ...
    for rrt_iter in range(int(iters)):

        # ------------ Sample random tip positions and their closest active states ------------    
        
        # Grab every active state

        #NOTE: questions =>
        # - how does propagation origin work now? (might be answered by adding random sampling)
        # - where is randomization for rrt() being done in sst()? <= look at sampling done for sst()

        active_states_mask = tree._isactive
        random_state_indices = sample_random_state(sst_params, batch_size, tree._tips[active_states_mask])
        propagate_origin_idx = np.where(active_states_mask)[0][random_state_indices]
        ...
        if sst_params.do_set_cover:
            ...
            if using_dynamic_obstacles: new_obj_positions = new_obj_positions[non_overlapping_mask]
            if using_dynamic_obstacles: new_obj_dstates = new_obj_dstates[non_overlapping_mask]
        ...
        for idx in in_goal_mask.nonzero()[0]:
            sst_params.solutions.put(DontCompareSecond(
                tiebreak_factor * length_unbatched(sim_params, new_cspaces[idx], new_bodies[idx]),
                {
        ...
        # Add new states:
        _ = tree.add_states(isactive=True,
                            c_space=new_cspaces,
                            dstate=new_dstates,
                            obj_positions=new_obj_positions if using_dynamic_obstacles
                                          else None,
                            obj_dstates=new_obj_dstates if using_dynamic_obstacles
                                          else None,
                            bodies=new_bodies,
                            time=new_times,
                            bending_control=current_bending_controls,
                            ...
                            tip=new_tips,
                            # Tree stuff
                            parent_idx=propagate_origin_idx,
                            num_children=0)

        # Update child counter for parents of recently added states
        tree._num_children[propagate_origin_idx] += 1
        ...
        draw_stats(sst_params, rrt_iter, iters, num_active_nodes, num_total_nodes)
        render()
...
def rrt_star(sst_params: SSTparams, sim_params: VineParams, initial_obj_pose=None, callback=None):

    # Potential FIXME: how will this run now that iters is static?
    # Do NOTE: prev sst_iter was used to squash iter/batch dim upon getting new state info
    rrt_iter = 1000

    # Initial tree is None, rrt will create it
    tree = None
    ...
    while True:
        ...
        tree, info = rrt(sst_params, sim_params, initial_obj_pose, tree, rrt_iter, callback)
...
if __name__ == "__main__":
    ...
    grow_rate_mps = 0.3 # FIXME: when this becomes too low, cspace_to_tip() stops working
    ang_damp = 5.0e-5
    lin_damp = 0.10
    dt_s = 1.0 / 90     # was 1.0 / 90
    ...
    if masses_kg.size > 0:
        ...
        initial_obj_pose = initial_obj_pose.cpu().numpy()
    else:
    ...
    rrt_star(sst_params, sim_params, initial_obj_pose=initial_obj_pose)
