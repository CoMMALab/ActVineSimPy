...
@profile
def sst(sst_params: SSTparams, sim_params: VineParams, tree, iters=1000, callback=None):
    ...
    # for wit_idx in range(tree.num_witnesses):
    #     if witness_has_rep_mask[wit_idx]:
    #         draw_witness_to_rep(tree, wit_idx)
        
    # Draw all parent-child edges for active states
    # for state_idx in range(tree.num_states):
    #     if tree._isactive[state_idx]:
    #         draw_parent_child_edge(tree, state_idx)
    ...
    for sst_iter in range(int(iters)):
        ...
        # draw_samples_and_active(xrand, tree._tips[propagate_origin_idx], sst_params.δs)
        
        # ------------- Monte Carlo propagation of the closest states -------------
        
        # Sample and set bending control
        # Can ben 180 deg in 1 meter, so 0.125 rad/2.5cm
        # FIXME can we do more? find empirically
        ...
        xnew_tips = cspace_to_tip(sim_params, xnew_cspaces.shape[0], xnew_cspaces, xnew_bodies, init_x, init_y, init_heading) 
        ...
        xnew_costs_come = np.tile(cost_come, [steps_to_iter])
        ...
        assert xnew_costs_come.shape == (steps_to_iter * batch_size,)
        ...
        # Now, after the propagate, we have the following xnew states
        #   - xnew_cspaces: shape (steps * B, max_bodies + 1)
        #   - xnew_bodies: shape (steps * B,)
        #   - xnew_times: shape (steps * B,)
        #   - xnew_tips: shape (steps * B, 3)
        ...
        if sst_params.do_set_cover:
            print('starting max_cover with', xnew_tips.shape[0], 'states')
            start_time = time.time()
            non_overlapping_mask = max_cover(sst_params, np.asarray(xnew_tips)) # FIXME jaxify, handle no-copy
            print('max_cover time:', time.time() - start_time, 'ended with', non_overlapping_mask.sum(), 'states')
            
            xnew_cspaces = xnew_cspaces[non_overlapping_mask]
            xnew_bodies = xnew_bodies[non_overlapping_mask]
            xnew_times = xnew_times[non_overlapping_mask]
            xnew_tips = xnew_tips[non_overlapping_mask]
        ...
        if sst_params.do_cost_to_go:
            xnew_costs_total = xnew_costs_come + geometric_cost_to_go(sst_params, xnew_tips) + \
                            tiebreak_factor * length(sim_params, xnew_cspaces, xnew_bodies)
        else:
        ...
        in_goal_mask = np.linalg.norm(xnew_tips[:, 0:2] - sst_params.goal[0:2], axis=1) < sst_params.goal_radius
        for idx in in_goal_mask.nonzero()[0]:
            sst_params.solutions.put(DontCompareSecond(
                xnew_costs_come[idx].item() + tiebreak_factor * length_unbatched(sim_params, xnew_cspaces[idx], xnew_bodies[idx]),
                {
                    'cspace': xnew_cspaces[idx],
                    'bodies': xnew_bodies[idx],
                    'bending_control': current_bending_controls[idx],
                    'cost_to_come': xnew_costs_come[idx],
                    'cost_total': xnew_costs_total[idx],
                    'tip': xnew_tips[idx],
                }
            ))
        ...
        xnew_fresh_mask, xnew_dominating_states_mask, xnew_dominating_states_witness_idx = \
        ...
        draw_tips(xnew_tips, costs=xnew_costs_total)
        
        # Assert the positions of new witness states do not intersect with previous witness
        # _, min_tip_dist = nearest_neighbor(sst_params, tree.witness_positions(), xnew_tips[xnew_fresh_mask])
        ...
        xnew_fresh_idx = tree.add_states(isactive=True,
                                        ...
                                        tip=xnew_tips[xnew_fresh_mask],
                                        # Tree stuff
                                        parent_idx=propagate_origin_idx[xnew_fresh_mask],
                                        num_children=0)
        
        # Add new dominating states to the tree, and record their indexes
        xnew_dominating_states_idx = tree.add_states(isactive=True,
                                        c_space=xnew_cspaces[xnew_dominating_states_mask],
                                        ...
                                        # Tree stuff
                                        parent_idx=propagate_origin_idx[xnew_dominating_states_mask],
                                        num_children=0)
        ...
        # for idx in xnew_fresh_idx:
        #     draw_parent_child_edge(tree, idx)

        # for idx in xnew_dominating_states_idx:
        #     draw_parent_child_edge(tree, idx)
            
        # Update child counter for parents of recently added states
        tree._num_children[propagate_origin_idx[xnew_fresh_mask]] += 1
        tree._num_children[propagate_origin_idx[xnew_dominating_states_mask]] += 1
            
        # For each fresh state, create a new witness and assign state as rep
        new_witness_idx = tree.add_witnesses(xnew_tips[xnew_fresh_mask], xnew_fresh_idx)
        ...
        old_rep_idx = tree._rep_idxs[xnew_dominating_states_witness_idx]
        ...
        # Set the new rep
        tree._rep_idxs[xnew_dominating_states_witness_idx[xnew_dominating_states_mask]] = xnew_dominating_states_idx
        
        # Prune the old rep, and any valid ancestors
        for idx in old_rep_idx:
            prune_path_at(idx, tree)
        
        # for idx in xnew_dominating_states_witness_idx:
        #     draw_witness_to_rep(tree, idx)
...
if __name__ == "__main__":
    ...
    # ------ Generate new config ------
    # cfg = load_box_config('envs/env3.txt')
    # cfg = generate_box_config()
    # save_box_config('envs/env_sst_test.txt', cfg)
    ...
    # sst(sst_params, sim_params, tree=None, iters=1000)
