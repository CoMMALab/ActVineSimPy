...
#NOTE: when cleaning up code, you can go through functions and remove explicit calls to this
...
def extend_cspace(params: VineParams, cspace: torch.tensor, dstate: torch.tensor, n_bodies:int):
    ...
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    cspace = cspace.to(device)
    dstate = dstate.to(device)
...
def dynamic_obj_sdf_measure(dynamic_obj_positions: torch.tensor, params: VineParams):
    ...
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    dynamic_obj_positions = dynamic_obj_positions.to(device)
    ...
    all_rects = torch.cat([torch.tensor(params.obstacle_rects, device=device), 
                           dynamic_obj_positions], axis=0)
...
def joint_measure(c_space: torch.tensor,
    ...
    '''
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    c_space = c_space.to(device)

    # constraints = torch.zeros(n_bodies * 2)
    constraints = torch.zeros(params.max_bodies * 2, device=device)
    
    # xs = c_space[:n_bodies, 0]
    # ys = c_space[:n_bodies, 1]
    # thetas = c_space[:n_bodies, 2]

    idx = torch.arange(c_space.shape[0], device=device)
    mask = idx < n_bodies
    ...
    torch_set_in_1D(constraints, 1,
                    (torch_get_from_1D(ys, 0) - y0) - params.radius * \
    ...
    idx = torch.arange(constraints.shape[0], device=device)
...
def proximity_measure(cspace: torch.tensor,
    ...
    def get_overlap_measure(joint_center_coords: tuple[2], joint_radius: float,
        ...
        depth = torch.tensor(joint_radius, device=device) - dist
...
def growth_measure(cspace: torch.tensor,
    ...
    def torch_get_from_2D(tensor: torch.tensor, row_idx, col_idx):
        ...
        '''
        idx = torch.arange(tensor.shape[0], device=device)
        mask = (idx == row_idx)
...
def get_bending_energy(params: VineParams, cspace: torch.tensor, bend_params: torch.tensor, bend_energy_func: Callable):
    ...
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    cspace = cspace.to(device)
    bend_params = bend_params.to(device)
...
def get_object_motion(params: VineParams, dstate: torch.tensor, weight: float):
    ...
    dyn_mass_tensor = torch.tensor(params.dynamic_objs_mass, device=device)
...
def compute_jacobians(params: VineParams, 
    ...
    growth_wrt_state, growth_wrt_dstate = torch.func.jacrev(partial(growth_measure, n_bodies=n_bodies),
                                         argnums=(0, 1))(
        cspace, dstate
    )
    ...
    forces = torch.zeros(cspace.shape[0] + dynamic_obj_positions.shape[0], 3, device=device)
    ...
    idx = torch.arange(forces.shape[0], device=device)
    mask_n = (idx < n_bodies)
...
def SCS_solve_layers(mass_matrix, forces_info,
    ...
    cspace_sdf_now = cspace_sdf_now.cpu()
...
def SCS_solve(params: VineParams, dstate, forces,
    ...
    if cvxpylayer is None:
        ...
        cspace_sdf_constraint = cp.Parameter(cspace_sdf_con_term.shape[1:])
        dynamic_sdf_constraint = cp.Parameter(dynamic_sdf_con_term.shape[1:])
        proximity_constraint = cp.Parameter(proximity_con_term.shape[1:])
...
def SCS_step_vine(params: VineParams, cspace: np.array, dstate: np.array,
    ...
    global first_call_after_init

    # BATCHED step for simulator

    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

    cspace = torch.tensor(cspace, dtype=torch.float32, device=device)
    dstate = torch.tensor(dstate, dtype=torch.float32, device=device)
    n_bodies = torch.tensor(n_bodies, dtype=torch.int32, device=device)
    dynamic_obj_positions = torch.tensor(dynamic_obj_positions, dtype=torch.float32, device=device)
    ...
    next_dstate_solution = next_dstate_solution.to(device)
    
    if first_call_after_init: first_call_after_init = False
    
    # Use found dstate to find new positions:

    # Potential FIXME: why is detach() being used here in DiffVine?

    next_dstate_cspace = next_dstate_solution[:, :params.max_bodies + 1, :].detach()
    
    next_dstate_dynamic_objs = next_dstate_solution[:, params.max_bodies + 1:, :].detach()
